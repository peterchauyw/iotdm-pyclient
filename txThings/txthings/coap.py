...
class Options(object):
    ...
    def _setUriHost(self, segments):
        """Convenience setter: option"""
        if isinstance(segments, basestring): #For Python >3.1 replace with isinstance(segments,str)
            raise ValueError("Host should be passed as a list or tuple of segments")
        self.deleteOption(number=URI_HOST)
        for segment in segments:
            self.addOption(StringOption(number=URI_HOST, value=str(segment)))

    def _getUriHost(self):
        """Convenience getter: option"""
        segment_list = []
        uri_host = self.getOption(number=URI_HOST)
        if uri_host is not None:
            for segment in URI_HOST:
    ...
    def _setUriPort(self, port):
        """Convenience setter:  option"""
        self.deleteOption(number=URI_PORT)
        self.addOption(UintOption(number=URI_PORT, value=port))
    ...
    def _setOneM2M_FR(self, segments):
        """Convenience setter: option"""
        if isinstance(segments, basestring): #For Python >3.1 replace with isinstance(segments,str)
            raise ValueError("FR should be passed as a list or tuple of segments")
        self.deleteOption(number=ONEM2M_FR)
        for segment in segments:
            self.addOption(StringOption(number=ONEM2M_FR, value=str(segment)))

    def _getOneM2M_FR(self):
        """Convenience getter: option"""
        segment_list = []
        oneM2M_FR = self.getOption(number=ONEM2M_FR)
        if oneM2M_FR is not None:
            for segment in oneM2M_FR:
                segment_list.append(segment.value)
        return segment_list

    oneM2M_FR = property(_getOneM2M_FR, _setOneM2M_FR)




    def _setOneM2M_RQI(self, segments):
        """Convenience setter: option"""
        if isinstance(segments, basestring): #For Python >3.1 replace with isinstance(segments,str)
            raise ValueError("RQI should be passed as a list or tuple of segments")
        self.deleteOption(number=ONEM2M_RQI)
        for segment in segments:
            self.addOption(StringOption(number=ONEM2M_RQI, value=str(segment)))

    def _getOneM2M_RQI(self):
        """Convenience getter: option"""
        segment_list = []
        oneM2M_RQI = self.getOption(number=ONEM2M_RQI)
        if oneM2M_RQI is not None:
            for segment in oneM2M_RQI:
                segment_list.append(segment.value)
        return segment_list

    oneM2M_RQI = property(_getOneM2M_RQI, _setOneM2M_RQI)



    def _setOneM2M_NM(self, segments):
        """Convenience setter: option"""
        if isinstance(segments, basestring): #For Python >3.1 replace with isinstance(segments,str)
            raise ValueError("NM should be passed as a list or tuple of segments")
        self.deleteOption(number=ONEM2M_NM)
        for segment in segments:
            self.addOption(StringOption(number=ONEM2M_NM, value=str(segment)))

    def _getOneM2M_NM(self):
        """Convenience getter: option"""
        segment_list = []
        oneM2M_NM = self.getOption(number=ONEM2M_NM)
        if oneM2M_NM is not None:
            for segment in oneM2M_NM:
                segment_list.append(segment.value)
        return segment_list

    oneM2M_NM = property(_getOneM2M_NM, _setOneM2M_NM)




    def _setOneM2M_TY(self, oneM2M_TY):
        """Convenience setter:  option"""
        self.deleteOption(number=ONEM2M_TY)
        self.addOption(UintOption(number=ONEM2M_TY, value=oneM2M_TY))

    def _getOneM2M_TY(self):
        """Convenience getter:  option"""
        oneM2M_TY = self.getOption(number=ONEM2M_TY)
        if oneM2M_TY is not None:
            return oneM2M_TY[0].value
        else:
            return None

    oneM2M_TY = property(_getOneM2M_TY, _setOneM2M_TY)
